...
#
# **当初の理由は誤りだった。** 「BSD sed は `\t` をタブとして解釈しない」として sed を
# 廃したが、macOS 26.5.2 の /usr/bin/sed で測ると**ブラケット式の外の `\t` はタブとして
# 効く**（#296）。効かないのはブラケット式の中だけで、ここで使っていた `\t` は外側
# だった。この形を残しても macOS で壊れない。
#
# それでも awk 側で出力する形を維持するのは、パイプが 1 本減って読みやすく、
# ファイル名・行番号・内容を 1 か所で組み立てられるため。移植性は理由ではない。
#
...
table_blocks() {
  md_file="$1" awk '
    ...
    #
    # 間隔指定 {0,3} を使わず 4 通りを並べている。**当初の理由（interval expression は
    # awk 実装ごとに対応が分かれる）は、対象プラットフォームでは成立しない。** macOS
    # 26.5.2 の /usr/bin/awk（version 20200816）でも devcontainer の mawk でも
    # 間隔指定は機能する（#296 の測定）。この書き方を維持するのは、4 通りの列挙が
    # 短く読み違えようが無いためで、移植性は理由ではない。
