...
# 確認）。区間指定 `{1,3}` は **下限が 1 なので** このリポジトリが対象とする 2 種類の
# awk（macOS の BWK awk / devcontainer の mawk）のどちらでも機能し、移植性は理由に
# ならない（#296 の実測。mawk では 1〜3 回に一致し 4 回に一致しないことを確認済み）。
#
# **下限が 2 以上の形は別である。** mawk は下限を超える繰り返しに一致しない（`^x{2,3}$`
# は xxx に一致しない）。scripts/check-shell-portability.sh が AWK_INTERVAL として
# 検出する。ここで `{1,3}` を使うぶんには影響しないが、**この判断を下限 2 以上へ
# 広げないこと。**
# それでも列挙する（`/^#[ \t]/ || /^##[ \t]/ || /^###[ \t]/`）のは、区間指定より
...
# scripts/check-table-breaks.sh の fence_len() と同じ規則）。フェンス内の行は
