...
it "同じ判定が、実行ビットの無いファイルを 100644 と読む（意図的な負例）"
# 「判定が存在する」ことと「判定が効いている」ことは別である。追跡下にあり、かつ
# 実行ビットを持たないファイルを同じ関数へ通し、100755 以外を返すことを見る。
...
$(cd "$REPO_ROOT" && git ls-files -- '*.md' | head -5)
PLAINEOF
if [[ -z "$NONEXEC" ]]; then
  fail "実行ビットの無い追跡ファイルを 1 件も見つけられない（対照群が成立しない）"
else
  pass
fi
