...
## 7. カタログ更新ルール（このリポジトリ適用）

- `docs/` 配下のファイルを追加・削除・改名した場合は、`docs/CATALOG.md` を同一コミットで更新すること。
- `scripts/` 配下のファイルを追加・削除・改名した場合は、`scripts/CATALOG.md` を同一コミットで更新すること。
- 可変ログ領域（例: `scripts/orchestration/runtime/**`, `scripts/orchestration/command-history/**`）を `scripts/CATALOG.md` の列挙対象から除外する場合は、その除外方針を `scripts/CATALOG.md` に明記すること。
