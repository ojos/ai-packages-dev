...
it "先頭 | の無い区切り行を使う表は通る（対照群、GFM は妥当な表として描画する）"
# `| a | b |` の次の行が `--- | ---`（先頭 | 省略）でも GFM は妥当な表として描画する。
# 区切り行の直後のデータ行を「新しい表の先頭」と誤認していないかを見る。
repo="$(new_repo)"
add_md "$repo" "nopipe-delim.md" '| a | b |
--- | ---
| 1 | 2 |
| 3 | 4 |
'
...
# ── 陽性: 正しい表のデータ行の直後に無関係な水平線があり、続く残骸を見落とす形 ──
#
# 少数意見の第二意見が実際に指摘した形（コーディネーターが実測で確認）。
# データ行を読んでいる区間（BODY）でも「直前が区切り行の形」を無条件の継続と
# みなすと、データ行の直後に来た無関係な水平線を表の区切り行と誤認し、その先の
# 残骸を継続行として見落とす。上の「表と無関係な水平線の直後」は水平線が
# ヘッダー行の**前**に来る形、こちらはデータ行の**後**に来る形で、状態を
# 区別しないと同じ穴が残る。

it "正しい表のデータ行の直後に無関係な水平線が続くと、その先の残骸を報告する"
repo="$(new_repo)"
add_md "$repo" "after-data-hr.md" '| a | b |
|---|---|
| 1 | 2 |
---
| 残骸 |
'
run_check "$repo"
assert_fail "データ行直後の水平線に続く残骸" "after-data-hr.md"
