...
it "全モードで verify / acceptance / loop-gate が生成される"
missing=""
for m in minimal standard full; do
  out="$(new_workdir)/p"
  run_bootstrap "$out" --mode "$m" >/dev/null 2>&1
  for s in verify.sh acceptance.sh loop-gate.sh; do
    [[ -f "$out/scripts/$s" ]] || missing="$missing $m:$s"
  done
done
...
out="$(new_workdir)/p"
run_bootstrap "$out" --mode minimal >/dev/null 2>&1
bad=""
for s in verify.sh acceptance.sh loop-gate.sh; do
...
out="$(new_workdir)/p"
run_bootstrap "$out" --languages node --mode minimal >/dev/null 2>&1
...
out="$(new_workdir)/p"
run_bootstrap "$out" --languages go --mode minimal >/dev/null 2>&1
...
out="$(new_workdir)/p"
run_bootstrap "$out" --languages node,rust --mode standard >/dev/null 2>&1
