...
#   1 件も走らない。バイナリを大量に追跡すると 2 段目の呼び出し回数がその件数に比例
#   するので、目に見えて遅くなったら拡張子による短絡を 1 段目の手前へ足すこと。
#
# 禁止バイトの渡し方:
...
# 正常。1 も受け入れる: GNU xargs は複数バッチに分割したとき、どのバッチも子の
# 終了コードが 1（該当なし）以外を返さなければ 123 を返す一方、実装によっては
# 単一バッチで完結する場合や移植先の xargs 実装差により、子の終了コード 1 が
# そのまま伝播することがある。**1 と 123 はどちらも「該当なし」を意味する点で
# 同じ**であり、これを異常とみなすと誤って失敗させてしまう。
# grep のエラー（終了コード 2）はどちらの値にも潰れないため、標準エラーが空でない
# ことをもって「検査が成立しなかった」と判定する（この判定は下の終了コード判定より
# 前に行う。標準エラーが実際に何か出ているのに、それを終了コードの解釈だけで
# 隠さないため）。
grep_status=0
...
if [[ "$grep_status" -ne 0 && "$grep_status" -ne 1 && "$grep_status" -ne 123 ]]; then
