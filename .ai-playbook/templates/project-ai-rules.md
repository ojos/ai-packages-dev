# プロジェクト共通 AI ルール

このファイルはプロジェクト共通ルールの正本です。
全体共通ルールを、このプロジェクトの事情に合わせて具体化します。

## この雛形の読み方

**書かれていない項目は「未定」ではありません。「全体共通ルールの既定に従う」という宣言です。**

そのため、**この雛形を 1 文字も変えずに使っても正本として成立します。** 実際、固有ルールをこのファイルへ集めるプロジェクトと、別の文書へ集めるプロジェクトの両方があり、どちらも成立しています。書き換えるのは、既定と違うことをする項目だけで足ります。

**既定値が書いてある項目**（`` `.env.example` `` のように具体名があるもの）は、devcontainer-bootstrap の生成物の既定です。違うものを使うなら書き換えてください。

**`（…を記載）` の形で残っている項目**は、**既定が原理的に決まらないもの**です。プロジェクトごとに違い、共通の既定を置けません。書かないままにすると、その項目については**規範が何も言っていない**状態になります。

### この雛形が保証しないこと

- **書かれた内容が妥当であることは保証しません。** 機械で見られるのは「**残っている空欄が『既定はありません』と宣言していること**」と「既定値として挙げたパスが実在すること」までです。**空欄が残っていること自体は検査しません**——既定が決まらない項目は、欄のまま残すのが正しい形です
- **2 層目が実際に書かれることは保証しません。** 規範は呼びかけであり、迂回できます（`.ai-playbook/shared-ai-rules.md` 12 章）
- **「埋めた風」の記述は通ります。** ただし上記のとおり、**埋めないこと自体は問題ではありません**

## 参照先

- 全体共通ルール: `.ai-playbook/shared-ai-rules.md`
- ロール責務: `.ai-playbook/role-contracts/`
- タスク手順: `.ai-playbook/task-playbooks/`
- レビュー運用: `.ai-playbook/review-workflow.md`
- intake 規律・判定根拠: `.ai-playbook/intake/`

実行環境の入口ファイル（`CLAUDE.md` 等）はこのファイルを参照し、最小差分のみを記述します。

## このプロジェクト固有の値

- プロジェクト固有の制約・検証手順: （記載。**既定はありません**。無ければこの節ごと消さずに空のまま残してください——「書いていない」ことが読み取れなくなります）

## 機密の具体化

共通規範「機密の取り扱い」を、このプロジェクトで具体化します。

- 機密の読み取り元: `.env`（シークレット管理サービスを使うなら書き換える。**このパスは生成物に含まれません**——利用者が作ります）
- 追跡除外の対象: `.env`（同上）
- 共有する雛形: `.env.example`（DCB の生成物の既定。値を持たない）

**機械化された検知層があります。** `scripts/check-no-secrets.sh` が、機密を含みやすい名前のファイルを「追跡対象へ入る前」と「追跡済み」の 2 経路で検査し、あわせて `.env.example` に機密の値が入っていないことと、`.env` とのキー整合を検査します。`scripts/verify.sh` が受け入れ条件の手前で呼ぶため、受け入れ条件を書き換えても検査は残ります。

```bash
bash scripts/check-no-secrets.sh   # 終了コード 0 / 標準出力 SECRETS_PASS
```

- **検知層であって、機密を置いてよい根拠ではありません。** 名前で判定するため、規約外の名前を付けた機密は捕まりません。上の 3 項目をプロジェクトで具体化し、`.gitignore` で除外することが一次の対策です。
- **git を前提にします。** 作業ツリーでない、追跡ファイルが 1 件も無い、といった「検査が成立しない」状態は合格にせず失敗させます（検査していないことと、機密が無いことは別です）。
- **検知した場合の対処**: 追跡前なら `.gitignore` へ加える。追跡済みなら `git rm --cached` で外す。既にコミット済みなら、削除コミットでは漏洩は解消しないため、履歴からの除去と当該資格情報の失効・再発行まで行う。

### GitHub 認証（gh）だけを例外にする理由

トークンをファイルへ書き写さず、ツール自身のログイン状態に持たせるのが原則です。**GitHub CLI だけはこの原則の例外**として、PAT（personal access token）を `.env` の `GH_TOKEN` へ置くことを認めます。

- **理由**: gh の OAuth App には「ユーザー × アプリ × scope あたり 10 トークン」の上限があります。上限に達した状態でどこかの環境が認証すると、GitHub が既存のトークンを 1 本破棄します（理由コード `max_for_app`）。溜まる単位は環境ではなく**認証の回数**で、`gh auth login` も `gh auth refresh` も自分の古い枠を返しません。失効に気づいた環境が再認証し、それがまた別の環境を失効させる連鎖になるため、**運用ルールでは回避できません**。PAT は OAuth App の認可ではないため、この枠の外にあります。
- **名前**: `GH_TOKEN`（gh 自身が読む名前）をそのまま使います。`GIT_IDENTITY_*` を別名にしているのと方針が逆に見えますが、理由が違います。git は自身が読む名前（`GIT_AUTHOR_EMAIL` 等）を環境へ置くと `user.useConfigOnly` の保護が無効になるため別名にします。gh には、環境変数を置くことで無効化される保護がありません。
- **空でも壊れません**: `GH_TOKEN` が空の環境は従来どおり保存済み認証で動きます。
- **設定中は `gh auth login` を実行しません**: env が優先されるためログイン結果は使われず、それでも OAuth トークンは 1 本発行されます。上限に達していれば、他環境のトークンを 1 本失効させるだけの結果になります。これは制約ではなく安全装置として扱います。

発行手順と必要権限は、対象リポジトリと行う操作によって変わります。**この雛形では決め打ちせず、プロジェクトごとに次を記述します。**

- PAT の発行手順: （種別・対象・有効期限を記載。**既定はありません**）
- 必要な権限: （行う操作に対する最小の権限セットを記載。**既定はありません**）
- 失効時・期限切れ時の再発行手順: （記載。**既定はありません**）

## 生成物の具体化

- コミットしない生成物: （記載。**既定はありません**。例: ビルド成果物、メディアファイル）
- 再生成手順: （コマンドを記載。**既定はありません**）

## 作業状況の記録先

共通規範「作業状況の記録」を、このプロジェクトで具体化します。
単一ファイルへの集中更新は並列実行と衝突するため、追記のみの形式や作業単位ごとの分割を検討します。

- 未完了の作業: （記録先を記載。**既定はありません**）
- 完了した作業の履歴: （記録先を記載。**既定はありません**）

## 外部サービスの状態管理

共通規範「外部サービスの状態管理」を、このプロジェクトで具体化します。

- 対象の外部状態: （記載。**既定はありません**。例: クラウドリソース、公開リポジトリ、リリース）
- 宣言・適用の手段: （記載。**既定はありません**。例: Terraform、専用スクリプト）
- 手動操作の扱い: 状態確認・調査に留め、恒久的な変更は宣言側を通す

### 外部層の受け入れ検証

宣言と実際の外部状態が一致しているかを、機械判定できる形で確かめます（`.ai-playbook/loop-workflow.md`「受け入れ条件の二層」）。

- 実行体: `scripts/acceptance-remote.sh`（DCB が `--with-aws` / `--with-gcp` を選んだときに配置する雛形）
- 起動方法: `VERIFY_ACCEPTANCE=scripts/acceptance-remote.sh bash scripts/verify.sh`
- 検証する内容: （記載。**既定はありません**。例: 宣言の差分検出が差分なしを返すこと、宣言したリソースが実在すること）
- 通す契機: 外部状態の宣言を変更したとき。**反復のたびに回す層ではなく、ローカル事前ゲートにも含めません**（下記「レビューの起動方法」）。
- 前提: 対象サービスへ認証済みであること。**この検証は認証を行いません**（資格情報をスクリプトへ書き写す経路を作らないため）。未認証やオフラインでの失敗は、宣言と外部状態の乖離ではありません。

## レビューの起動方法

共通規範「レビューワークフロー」のクロスモデル二段ゲートを、このプロジェクトで具体化します。

1. 主レビュー: （実装したモデル自身によるレビュー手段を記載。**既定はありません**——実装に使うモデルによって変わります）
2. 第二意見: `scripts/second-opinion-review.sh`（DCB の生成物の既定。`--engine` で実行 CLI を選ぶ）
3. 単一入口: `scripts/loop-gate.sh`（DCB の生成物の既定。commit identity 検証・受け入れ検証・第二意見を直列化する。push / PR 作成の前にこれを通します）

- 品質整理（任意の前段。主レビューの前に 1 回）: （起動手段と、このプロジェクトで「コード」とみなす範囲を記載。**既定はありません**——主レビューと同じく実装に使うモデルによって変わります。位置・回数・対象の制約は `.ai-playbook/review-workflow.md`「品質整理（任意の前段）」に従い、単一入口には含めません）
- 3 つを別々に思い出す運用は破綻するため、単一入口へまとめることを推奨します。実行体は利用側で用意します（このパッケージは実行ランタイムを持ちません）。
- 両段とも対象は致命バグ・脆弱性・型エラー・エッジケースの見落としに限ります。
- 修正は 1 イテレーションで完結させます。

### 受け入れ検証の二層

上記 1 の受け入れ検証は 2 層に分かれます（`.ai-playbook/loop-workflow.md`「受け入れ条件の二層」）。**単一入口が直列化するのはローカル層だけです。**

- ローカル層: `scripts/acceptance.sh`（DCB の生成物の既定。`scripts/verify.sh` が呼ぶ。ネットワークも外部認証も要さない検証で、ループの接地信号）
- 外部層: `scripts/acceptance-remote.sh`（同上。宣言と実際の外部状態の一致を検証。実行体と起動方法は上記「外部層の受け入れ検証」に書きます）
- **外部層を単一入口へ含めない理由**: 外部認証の失効やオフラインでゲート全体が止まり、**実装が正しいのにループが止まります。** 単一入口の目的は複数の段を思い出す運用を機構で塞ぐことであって、外部の可用性をゲートの前提条件に持ち込むことではありません。
- 外部層を通す契機: 外部状態の宣言を変更したとき（既定。違うなら書き換える）

### リモート最終ゲート

push / PR 作成後の最終ゲートを、このプロジェクトで具体化します（`.ai-playbook/review-workflow.md`「リモート最終ゲート」）。

- 手段: `.github/workflows/copilot-review.yml`（要求側）と `.github/workflows/review-gate.yml`（確認側）の 2 本（DCB の `--with-copilot-review` が配置する既定。PR 作成時に 1 回だけ要求する）
- **要求側と確認側の 2 本で 1 組です。** 確認側だけを省くと、要求側の契機が届かなかったときに最終ゲートが黙って抜けます（`.ai-playbook/review-workflow.md`「要求されたことを別の契機で確認する」）。確認側は要求しません。
- ローカル装備の `--with-copilot`（CLI・拡張・設定の永続化）ではワークフローは配置されません。効く場所が違うため別のフラグです。
- 要求は 1 回に限定します。機構で自動要求する場合は、再要求されないイベント（例: `pull_request` の `opened` のみ）に限定します（`.ai-playbook/review-workflow.md`）。
- 前提条件・失敗時の扱い: リポジトリ所有者の Copilot code review が有効であること（既定の手段を使う場合。有効でないと要求が 422 で失敗する）
